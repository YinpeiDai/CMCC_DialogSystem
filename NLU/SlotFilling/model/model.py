...
class RequestableSlotDector:
    """
    一个简单的 CNN
    对每个 requestable slots 都有一个 dector
    """
    def __init__(self, name,
                 max_sent_length=40,
                 word_embed_size=20,
        ...
        with tf.device('/gpu:0'), tf.variable_scope(name_or_scope=self.name,
            ...
            with tf.variable_scope("predict"):
                # predict
                self.predict = tf.argmax(self.final_output_logits,1)
                # print(self.predict.get_shape())
                self.correct = tf.equal(tf.cast(self.predict, tf.int32),
                                        tf.cast(self.output, tf.int32))
                self.accuracy = tf.reduce_mean(tf.cast(self.correct, tf.float32))
...
if __name__ == '__main__':
    informable_slots = InformableSlotDector(name='Cost',
                                            max_sent_length=30,
                                            max_word_length=20,
                                            word_embed_size=20,
                                            char_embed_size=20)
    requestable_slots = RequestableSlotDector(name='HAHA',
                                              max_sent_length=30,
                                              word_embed_size=20,
                                              char_embed_size=20)